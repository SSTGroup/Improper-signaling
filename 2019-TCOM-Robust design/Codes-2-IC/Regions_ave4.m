...
h21 = (1/sqrt(2))*(randn(1,iter)+1i*randn(1,iter));  % Secondary Channel Gain
H21 = abs(h21).^2;

h22 = (1/sqrt(2))*randn(1,iter)+1i*randn(1,iter);  % Secondary Channel Gain
H22 = abs(h22).^2;
% ==========================================
% ==========================================
...
for beta1=eps2%(dummy+1:end)
    dummy = dummy+1
    eps1=0;%sqrt(-beta1*log(1-Pro_in))
%     eps1 = sqrt(chi2inv(Pro_in,2)*(1/(1+1/beta1)));
    ...
    %                 cte = abs(q1_hat(ll,1)*h11(cnt)^2+q1_hat(ll,2)*h21_w ^2.*...
    %                     exp(1i.*phi_1)).^2;
    % 
    %                 cte1  = (cl(ll,1)*H11(cnt) + cl(ll,2)*abs(h21_w)^2 + sigma2).^2 ...
    %                     - max(cte);%abs(q1_hat(ll,1)*h11^2 + q1_hat(ll,2)*h21_w ^2).^2;
    % 
    %                 cte2 = (cl(ll,2)*abs(h21_w)^2 + sigma2).^2 ...
    %                     - abs(q1_hat(ll,2)*h21_w ^2).^2;
    % 
    ...
    % 
    %                 cte = abs(q1_hat(ll,2)*h22(cnt)^2+q1_hat(ll,1)*h12_w ^2.*...
    %                     exp(1i.*phi_2)).^2;
    %                 
    %                 cte1  = (cl(ll,2)*H22(cnt) + cl(ll,1)*abs(h12_w)^2 + sigma2).^2 ...
    %                     - max(cte);%abs(q1_hat(ll,2)*h22^2 + q1_hat(ll,1)*h12_w ^2).^2;
    % 
    %                 cte2 = (cl(ll,1)*abs(h12_w)^2 + sigma2).^2 ...
    %                     - abs(q1_hat(ll,1)*h12_w ^2).^2;
    % 
    %                 R2_i_n(ll) = .5 * log2(cte1/cte2); clear cte
        ...
        % =================================
        theta1 = beta1*pi;%2*asin(min(eps1/abs(h21(cnt)),1));
...
%         theta1 = theta1 + theta2;
        ...
        % ====================================================
        % ====================================================
        theta2 = beta1*pi;%2*asin(min(eps1/abs(h12(cnt)),1));
...
%%        
        ...
        %             p2 = max(p2,0);
            ...
            R2_imp(dummy,cnt)=R2_imp(1,cnt);
            Rth_2(dummy,cnt) =Rth_2(1,cnt);
        end
        % =====================================
  ...
  %%      
        ...
        %                 keyboard;
            ...
            R2_p(dummy,cnt)=R2_p(1,cnt);
            R1_p(dummy,cnt)=R1_p(1,cnt);
        end
