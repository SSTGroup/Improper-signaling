function [r, t_real] = r_MIMO_i_k(H)



global P  N_t N_r sigma  coun coun2 K sigma_d
...
for k=1:K
    ...
    eval(sprintf('C_z%i = sigma*eye(N_r,N_r)+sigma_d_i*temp;',k))
    ...
    for i=1:K
        if i~=k
            ...
            %--------------------------------------------
            eval(sprintf('b_%i_%i(:,:) = temp1 + sigma_d_i*temp2;',k,i));
        end
     end
end
% -----------------------------------------
    ...
    cvx_begin sdp quiet
       ...
       for k=1:K
           clear temp
            ...
            eval(sprintf('C_z_%i = sigma*eye(N_r,N_r)+sigma_d_i*temp;',k))
